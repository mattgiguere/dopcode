function ctio4k_init, obsnm, iss_nm, iss_bc, iss_obnm=iss_obnm,tag=tag, $
	date=date, dpad=dpad, avg=avg, mode=mode, psfmod=psfmod, shft_style=shft_style
...
if file_test('/Users/debrafischer/') then begin
  root_path='/Users/debrafischer/dop/'
  if date eq '11' then obs_dir = '/Volumes/LaCie/fitspec/' $
  else obs_dir = '/Volumes/LaCie/extra/mir7/fitspec/'+date+'/'    ; the fits files (obs) live here
endif;df
...
if mode eq 'narrow_slit' then begin
  m7_psfpix= [0.0, -4.8,-4.0, -3.2, -2.5, -1.8, -1.2, -0.8, -0.4, 0.4, 0.8, 1.2, 1.8, 2.5, 3.2, 4.0, 4.8]  
  m7_psfsig= [0.65,  1.4, 1.4,  1.3,  1.1,  0.9,  0.8,  0.5,  0.4, 0.4, 0.5, 0.8, 0.9, 1.1, 1.3, 1.4, 1.4]
  m7_ampl=[m7_psfsig[0],0.0,0.0,0.001,0.003,0.006,0.03,0.11, 0.2, 0.2, 0.11,0.03,0.007,0.002,0.001,0.0, 0.0]
endif
if mode eq 'slit' then begin
  m7_psfpix= [0.0, -5.0,-4.3, -3.6, -2.8, -2.2, -1.6, -1.0, -0.6, 0.6, 1.0, 1.6, 2.2, 2.8, 3.6, 4.3, 5.0]  
  m7_psfsig= [1.1, 1.6, 1.5,  1.4,  1.3,  1.0,  1.0,  0.8,  0.8, 0.8, 0.8, 1.0, 1.0, 1.3, 1.4, 1.5, 1.6]
  m7_ampl=[m7_psfsig[0],0.0,0.0,0.001,0.003,0.006,0.03,0.11, 0.2, 0.2, 0.11,0.03,0.007,0.002,0.001,0.0, 0.0]
endif
psfpix = m7_psfpix
psfsig = m7_psfsig
...
dopenv = dop_init(obsnm, iss_nm, iss_bc, iss_obnm = iss_obnm, bary_file = bary_file, del_ord = del_ord, $
			  ...
			  mode=mode, psfmod=psfmod, npsfpix=npsfpix, shft_style=shft_style, $
