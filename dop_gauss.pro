FUNCTION DOP_GAUSS, psfpar, dopenv, xarr=xarr, cntr=cntr

    o_pix = n_elements(xarr)           ; 121 subpixels = 30pix*osamp + 1
    osamp=dopenv.osamp
    ppix=dopenv.psfpix                 ; locations of Gaussians offset from central Gaussian
    psig=dopenv.psfsig                 ; width of offset Gaussians 
  ...
  ; CONSTRUCT AND ADD SURROUNDING GAUSSIANS
    for i=1,n_elements(psig)-1 do begin 
      ...
      if a_i gt 0 then begin        ; gaussian component exists 
         ...
         tmp_ip[xx]=tmp_ip[xx] + psfpar[i]*exp(-0.5 * ((xarr[xx]-cen) / a_i)^2)
      endif
    endfor 
