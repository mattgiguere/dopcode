pro dr_obs_loop, star=star,iss_nm=iss_nm, iss_obnm=iss_obnm,$
...
;  iss_obnm='rs17.124', tag='b',jdmin=11000.
  ...
  if ~keyword_set(dsst_tag) or ~keyword_set(iss_nm) then begin
     ans='' & read,'the default dsst_tag is n, is this OK? (y/n) ',ans
     if ans eq 'y' then dsst_tag = 'n'
...
;  tag='b'
  if ~keyword_set(jdmin) then jdmin=11000.

;  iss_nm='dssthip36616df_rs10.dat'
;  star='HIP36616'
;  iss_obnm='rs10.81'
  ...
  for j=0,num-1 do begin 
     dopenv=lick_init(obs_id[j], iss_nm, iss_bc, iss_obnm=iss_obnm,tag=tag)
     ...
     if nx eq 1 then begin
        bcat=bcat[x[0]]
        objnm = bcat.objnm
        found=findfile('/mir1/files/vd'+tag+objnm+'*'+obs_id[j], count=count)
        if count eq 1 then print, found
        ffits=findfile('/mir1/fitspec/'+obs_id[j]+'.fits',count=fcount)
