...
x=where(strmid(bcat.objnm,0,2) eq 'HR' and bcat.obtype eq 'o',nobs)
bcat=bcat[x]
...
; make a structure for batch_dop 
  catTmpl={name:'',dir:'/home/fischer/dop2/dop_results/',runnum:'',obsnm:'',$
 ...
 ; check for an existing catalog file and catalog lock
  ...
  printf,catlun, "Writing new stars to catalog (db_bstarI2.dat)"
  free_lun,catlun
  ...
  for i=0L,nobs-1 do begin
     ...
     if nx eq 0 then begin      ; not in cat
        cat[i].name = bcat[i].objnm
        cat[i].obsnm = bcat[i].obsnm
        cat[i].runnum=strmid(cat[i].obsnm,0,5)
        ff=findfile('/mir7/files/cdbb*_'+cat[i].obsnm,count=count)
        print,ff
