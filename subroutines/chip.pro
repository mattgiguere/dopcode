...
if letters ne 'rq' then numbers=fix(strmid(tpname,2,2))
if letters eq 'rq' then numbers=fix(strmid(tpname,3,2))
...
;CTIO
if letters eq 'rq' then begin
   run=strmid(tpname,2,1)
   numbers=fix(strmid(tpname,3,2))
   if run eq 'a' then dwr = 200
   if run eq 'b' and numbers lt 1600 then dwr = 200
   if run eq 'b' and numbers ge 1600 then dwr = 201
   if run eq 'c' then dwr = 201
   if dwr eq 200 then gain = 1.3  ; approx, ave of both amps
   if dwr eq 201 then gain = 4.5  ; approx, ave of both amps
endif
...
;remeasured the gain dewar 8 / 24 with richard stover on oct 26,2001
;gain for high resistivity CCD adjusted after testing
gain_24_tst=1.53
gain_24=6.1

if letters eq 'rs' and numbers lt 85 then begin
   dwr=18
   gain=2.80
endif
if letters eq 'rs' and numbers ge 85 then begin
   dwr=24
   gain=gain_24
endif
...
if letters eq 'rt' and numbers ge 1 then begin
   dwr=24
   gain=gain_24
endif
...
if letters eq 're' and numbers ge 11 and numbers le 15 then begin 
   dwr=24
   gain=gain_24
endif
;
...
if letters eq 'rp' then begin
   dwr=171       ;subaru/HDS chip
   gain=1.665     
endif
;
...
if letters eq 'rq' then dwr = 50
;
telescope = 'SHANE'                          ;Assume Lick 3-m
if letters eq 'rq' then telescope = 'CTIO'
...
if letters eq 'rd' then begin
  ...
  if numbers ge 40 then telescope = 'CAT'
  if numbers ge 42 then telescope = 'CAT'
...
if letters eq 'rt' then telescope = 'CAT'     ;Assume CAT observation
